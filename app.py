...
def enriched_daily_intelligence():
    p=fusion()
    base={"ok":True,"version":"40.25","generated_at":p["generated_at"],
    ...
    try:
        ...
        base["consensus"]=con
        usable=[v for v in qs.values()
                if v["verified"] and v["risk_score"] is not None]
        if usable:
...
def log_prediction_snapshot(horizon_hours=24):
    ...
    intel=enriched_daily_intelligence()
    btc=_btc_spot()
...
def daily_intelligence():
    maintain_outcomes()
    return enriched_daily_intelligence()
