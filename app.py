...
UA={"User-Agent":"CryptoCopilot/40.25 personal research dashboard"}
...
# ---------- V40.25 RENDER-RESILIENT FALLBACKS ----------
...
def crypto_market():
    ...
    return cached("crypto_v425",30,work)
...
def cross_asset_quotes():
    ...
    return cached("cross_v425",300,work)
...
def gdelt():
    def work():
        ...
        return a
    return cached("news_v425",600,work)
...
# ---------- V40.25 VERIFIED SOURCE HARDENING ----------
...
_original_bls_calendar_v425 = bls_calendar
def bls_calendar():
    a = _original_bls_calendar_v425()
    if a.get("verified"):
        return a
    events=[]
...
_original_sec_crypto_v425 = sec_crypto
def sec_crypto():
    a = _original_sec_crypto_v425()
    if a.get("verified"):
        return a
    try:
...
_original_cross_asset_quotes_v425 = cross_asset_quotes
def cross_asset_quotes():
    # First retain the V40.25 chain (Stooq -> Treasury/VIX composite).
    base=_original_cross_asset_quotes_v425()
    ...
    # V40.25 base is still useful if it is the verified Treasury+Cboe composite.
    return base


# ---------- V40.25 SOURCE QUALITY + CONSENSUS + OUTCOME TRACKING ----------
# Adds:
# 1) source freshness/quality scores,
# 2) agreement/consensus measurement,
# 3) adaptive weighting based on source quality,
# 4) forward outcome snapshots to evaluate whether daily risk calls were useful.

SOURCE_DEFAULT_MAX_AGE_SEC = {
    "crypto_market": 120,
    "bls_macro": 86400 * 45,
    "treasury": 86400 * 3,
    "cboe_vix": 86400 * 3,
    "cross_asset": 3600 * 6,
    "bls_calendar": 86400 * 7,
    "fomc_calendar": 86400 * 7,
    "fed_rss": 3600 * 12,
    "sec": 3600 * 24,
    "gdelt": 1800,
}

def quality_from_source(name, payload):
    if not payload or not payload.get("verified"):
        return {"quality":0,"freshness":0,"confidence":0,"reason":"unverified"}
    source=(payload.get("source") or "").lower()
    summary=(payload.get("summary") or "").lower()
    # Base trust tier.
    if any(k in source for k in ("u.s. treasury","bureau of labor statistics","federal reserve","u.s. sec","cboe")):
        trust=95
    elif any(k in source for k in ("coinbase","binance","coingecko")):
        trust=90
    elif "google news" in source:
        trust=76
    elif "yahoo finance" in source or "stooq" in source:
        trust=72
    else:
        trust=68

    # Penalize partial/fallback modes, but keep them usable.
    if payload.get("partial"):
        trust -= 10
    if "fallback" in summary or "fallback" in source:
        trust -= 8
    if "snapshot" in summary or "snapshot" in source:
        trust -= 6

    # Most current source functions do not expose retrieval timestamps.
    # Use backend generation time and source-specific staleness rules.
    # Freshness therefore reflects feed category + whether the source is live/fallback.
    freshness=90
    if name in ("bls_macro","bls_calendar","fomc_calendar"):
        freshness=85
    elif name in ("treasury","cboe_vix"):
        freshness=88
    elif name in ("fed_rss","sec","gdelt"):
        freshness=86
    elif name=="crypto_market":
        freshness=98
    ...
    confidence=clamp(trust*0.62 + freshness*0.38, 0, 100)
    return {
        "quality":round(clamp(confidence,0,100),1),
        "freshness":round(clamp(freshness,0,100),1),
        "confidence":round(clamp(trust,0,100),1),
        "reason":"verified"
    }

def classify_risk_bucket(score):
    if score is None: return "unavailable"
    x=float(score)
    if x>=65: return "risk_off"
    if x<=35: return "risk_on"
    return "neutral"

def source_quality_snapshot():
    feeds={
    ...
    for k,v in feeds.items():
        q=quality_from_source(k,v)
        out[k]={
            "verified":bool(v.get("verified")),
            "risk_score":v.get("risk_score"),
            "bucket":classify_risk_bucket(v.get("risk_score")),
            "source":v.get("source"),
...
def consensus_metrics(qs):
    usable=[v for v in qs.values() if v["verified"] and v["risk_score"] is not None and v["quality"]>=45]
    if not usable:
        return {"agreement":0,"direction":"unavailable","dispersion":None,"usable_sources":0}
    buckets=[v["bucket"] for v in usable]
    counts={b:buckets.count(b) for b in ("risk_off","neutral","risk_on")}
    direction=max(counts,key=counts.get)
    agreement=100*counts[direction]/len(usable)
    scores=[float(v["risk_score"]) for v in usable]
    mean=sum(scores)/len(scores)
    dispersion=(sum((x-mean)**2 for x in scores)/len(scores))**0.5 if len(scores)>1 else 0
    return {
...
# Keep original fusion and wrap it with quality-aware diagnostics.
_original_daily_intelligence_v425 = daily_intelligence
def daily_intelligence():
    base=_original_daily_intelligence_v425()
    try:
        qs=source_quality_snapshot()
        con=consensus_metrics(qs)
        base["source_quality"]=qs
        base["consensus"]=con

        # Adaptive confidence: only modifies confidence/coverage metadata,
        # not the raw source values themselves.
        usable=[v for v in qs.values() if v["verified"] and v["risk_score"] is not None]
        if usable:
            avgq=sum(v["quality"] for v in usable)/len(usable)
            agreement=con.get("agreement",0)
            confidence=clamp(avgq*0.65 + agreement*0.35,0,100)
        else:
            confidence=0
        base["intelligence_confidence"]=round(confidence,1)

        # If disagreement is high, mark the environment as mixed rather than
        # overstate certainty.
        if con.get("usable_sources",0)>=4 and con.get("agreement",0)<50:
            base["consensus_warning"]="LOW SOURCE AGREEMENT"
        elif con.get("dispersion") is not None and con["dispersion"]>22:
            base["consensus_warning"]="HIGH RISK DISPERSION"
        else:
            base["consensus_warning"]=None
    except Exception as e:
        base["source_quality_error"]=str(e)
    return base

# --- Outcome tracking ---
# Store periodic predictions and later compare with BTC moves.
...
def _btc_spot():
    # Reuse public crypto source details if possible.
    try:
        cm=crypto_market()
        rows=((cm.get("details") or {}).get("rows") or [])
        for r in rows:
            if r.get("symbol") in ("BTCUSDT","BTC-USD"):
                # Coinbase fallback rows do not currently expose price.
                # Use Coinbase spot endpoint as simple independent price.
                break
        r=http_get("https://api.coinbase.com/v2/prices/BTC-USD/spot", timeout=6)
        r.raise_for_status()
        return float(r.json()["data"]["amount"])
    except Exception:
        return None

def log_prediction_snapshot(horizon_hours=24):
    _ensure_outcome_table()
    intel=daily_intelligence()
    btc=_btc_spot()
    risk=intel.get("risk_score")
    shield=intel.get("shield_action") or intel.get("action") or intel.get("environment")
    conf=intel.get("intelligence_confidence")
    con=sqlite3.connect(DB_PATH)
    try:
        # Avoid duplicate logs more often than every 30 minutes.
        row=con.execute(
            "SELECT created_at FROM prediction_outcomes WHERE horizon_hours=? ORDER BY id DESC LIMIT 1",
            (horizon_hours,)
        ).fetchone()
        if row:
            try:
                age=(now()-dtparser.isoparse(row[0])).total_seconds()
                if age < 1800:
                    return {"logged":False,"reason":"recent snapshot exists"}
            except Exception:
                pass
        con.execute(
            "INSERT INTO prediction_outcomes(created_at,horizon_hours,risk_score,shield,confidence,btc_price,status) VALUES(?,?,?,?,?,?,?)",
            (now().isoformat(),horizon_hours,risk,shield,conf,btc,"pending")
        )
        con.commit()
        return {"logged":True,"risk_score":risk,"confidence":conf,"btc_price":btc}
    finally:
        con.close()
...
def evaluate_prediction_outcomes():
    ...
    current=_btc_spot()
    if current is None:
        return {"evaluated":0,"reason":"BTC price unavailable"}
    con=sqlite3.connect(DB_PATH)
    try:
        rows=con.execute(
            "SELECT id,created_at,horizon_hours,btc_price FROM prediction_outcomes WHERE status='pending' AND btc_price IS NOT NULL"
        ).fetchall()
        n=0
        for rid,created,h,btc in rows:
            try:
                age=(now()-dtparser.isoparse(created)).total_seconds()/3600
                if age < h: continue
                ret=(current/float(btc)-1)*100
                con.execute(
                    "UPDATE prediction_outcomes SET future_price=?,realized_return=?,evaluated_at=?,status='evaluated' WHERE id=?",
                    (current,ret,now().isoformat(),rid)
                )
                n+=1
            except Exception:
                pass
        con.commit()
        return {"evaluated":n,"current_btc":current}
    finally:
        con.close()

def outcome_scorecard(limit=200):
    _ensure_outcome_table()
    con=sqlite3.connect(DB_PATH)
    try:
        rows=con.execute(
            "SELECT risk_score,confidence,realized_return FROM prediction_outcomes WHERE status='evaluated' AND realized_return IS NOT NULL ORDER BY id DESC LIMIT ?",
            (limit,)
        ).fetchall()
    finally:
        con.close()
    if not rows:
        return {"count":0,"status":"building history"}
    correct=0
    abs_err=[]
    for risk,conf,ret in rows:
        if risk is None: continue
        # Risk-off prediction considered directionally right if BTC fell;
        # risk-on considered right if BTC rose; neutral ignored for accuracy.
        if risk>=65:
            correct += 1 if ret<0 else 0
        elif risk<=35:
            correct += 1 if ret>0 else 0
        abs_err.append(abs(float(ret)))
    directional=sum(1 for r,_,_ in rows if r is not None and (r>=65 or r<=35))
    return {
        "count":len(rows),
        "directional_calls":directional,
        "directional_accuracy":round(100*correct/directional,1) if directional else None,
        "avg_abs_realized_move":round(sum(abs_err)/len(abs_err),2) if abs_err else None
    }

# Best-effort periodic tracking piggybacks on API requests.
def maintain_outcomes():
    try:
        evaluate_prediction_outcomes()
        log_prediction_snapshot(24)
    except Exception:
        pass
...
def fusion():
    ...
    out={"ok":True,"version":"40.25","generated_at":now().isoformat(),
...
app=FastAPI(title="Crypto Copilot Intelligence Backend V40.25",lifespan=lifespan)
app.add_middleware(CORSMiddleware,allow_origins=["*"],allow_credentials=False,allow_methods=["GET"],allow_headers=["*"])

@app.get("/health")
def health():
    return {"ok":True,"version":"40.25","time_utc":now().isoformat()}
...
def daily_intelligence():
    ...
    return {"ok":True,"version":"40.25","generated_at":p["generated_at"],
...
@app.get("/api/v1/source-quality")
def api_source_quality():
    maintain_outcomes()
    qs=source_quality_snapshot()
    return {"ok":True,"generated_at":now().isoformat(),"sources":qs,"consensus":consensus_metrics(qs)}

@app.get("/api/v1/outcome-scorecard")
def api_outcome_scorecard():
    maintain_outcomes()
    return {"ok":True,"generated_at":now().isoformat(),"scorecard":outcome_scorecard()}

@app.get("/api/v1/config")
def config():
    return {"version":"40.25","api_keys_required":False,
